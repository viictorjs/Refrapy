...
class Sisref(Frame):
    ...
    def fechar(self):
        ...
        if messagebox.askyesno("Refrapy - sisref", "Sair do programa?"):
    ...
    def abrirgp(self):
        ...
        if self.curvaExiste == True:
            ...
            try:
                ...
                arquivo = filedialog.askopenfilename(title='Abrir',filetypes=[('Refrapy pick','*.gp'),
                                                                    ('Todos os arquivos','*.*')])
            except:
    ...
    def showShots(self):
        ...
        if self.modeloExiste == True:
            ...
            messagebox.showinfo('Refrapy - Sisref', 'Nennhum modelo plotado!')
            ...
            def definir ():
                ...
                if len(entry_ymax.get()) > 0:
                    ...
                    if float(entry_ymax.get()) > self.ax_model.get_ylim()[0]:
                        ...
                        messagebox.showinfo('Refrapy - Sisref', 'Verifique os valores de y!')
                ...
                if len(entry_xmin.get()) > 0:
                    ...
                    if float(entry_xmin.get()) < self.ax_model.get_xlim()[1]:
                        ...
                        messagebox.showinfo('Refrapy - Sisref', 'Verifique os valores de x!')
        ...
        if self.curvaExiste == True:
            ...
            def click_gauss(event):
                ...
                if event.artist in bolas_gauss:
                    ...
                    z = -1*(m[len(self.fontes):-1]*(1/m1)*(1/m[-1]))/np.sqrt(((1/m[-1])**2)-(1/m1)**2)
            ...
            def click_solus(event):
                ...
                if event.artist in linhas_solus:
                    ...
                    for m in residuals.values():
                        ...
                        if sum(-1*(m[len(self.fontes):-1]*(1/m1)*(1/m[-1]))/np.sqrt(((1/m[-1])**2)-(1/m1)**2)) == sum(z):
                    ...
                    if len(entryIntervalo.get()) > 0:
                        ...
                        try:
                            ...
                            for i in range(len(R)):
                                ...
                                if b.get_offsets()[0][0] >= intervalo[0] and b.get_offsets()[0][0] <= intervalo[1]:
                                    ...
                                    esp, = ax_solus.plot(np.array(temp),
                                         -1*(residuals[b.get_offsets()[0][0]][len(self.fontes):-1]*(1/m1)*(1/residuals[b.get_offsets()[0][0]][-1]))/np.sqrt(((1/residuals[b.get_offsets()[0][0]][-1])**2)-(1/m1)**2),
                            ...
                            ax_solus.plot(np.array(temp),
                                          -1*(best_fit[len(self.fontes):-1]*(1/m1)*(1/best_fit[-1]))/np.sqrt(((1/best_fit[-1])**2)-(1/m1)**2),
                            ...
                            messagebox.showinfo('Refrapy - Sisref', 'Intervalo inválido. Entre com um valor entre 0 e 100!')
            ...
            def velModel():
                ...
                self.ax_model.plot(np.array(temp)[0],np.array(temp)[-1], color = '#5bdaff', label = str(int(round(1/m1*1000)))+' m/s',linewidth = 3)
                l_model,= self.ax_model.plot(np.array(temp),-1*(media_solus[len(self.fontes):-1]*(1/m1)*(1/media_solus[-1]))/np.sqrt(((1/media_solus[-1])**2)-(1/m1)**2),
                ...
                plt.gca().set_aspect('equal', adjustable='box')
                plt.draw()
                self.ax_model.fill_between(np.array(temp),-1*(media_solus[len(self.fontes):-1]*(1/m1)*(1/media_solus[-1]))/np.sqrt(((1/media_solus[-1])**2)-(1/m1)**2),
                                      0, color = '#5bdaff')
                self.ax_model.fill_between(np.array(temp),-1*(media_solus[len(self.fontes):-1]*(1/m1)*(1/media_solus[-1]))/np.sqrt(((1/media_solus[-1])**2)-(1/m1)**2),
            ...
            for sol in residuals.values():
                ...
                l_solu, = ax_solus.plot(np.array(temp),-1*(sol[len(self.fontes):-1]*(1/m1)*(1/sol[-1]))/np.sqrt(((1/sol[-1])**2)-(1/m1)**2),
    ...
    def invAuto(self):
        ...
        if self.curvaExiste == True:
            ...
            m1s = []
            G1s = {}
            for i in range(len(self.xDataCamada1)):
                if i+1 not in self.especiais:
                    G1s.update({i+1:np.zeros((len(self.xDataCamada1[i+1]),1))})
                else:
                    G1s.update({i+1:[{1:np.zeros((len(self.xDataCamada1[i+1][1]),1))},
                                    {2:np.zeros((len(self.xDataCamada1[i+1][2]),1))}]})
   
            for i in range(len(self.xDataCamada1)):
                if i+1 not in self.especiais:
                    for j in range(len(G1s[i+1])):
                        G1s[i+1][j] = abs(self.fontes[i+1]-self.xDataCamada1[i+1][j])
                else:
                    
                    for k in range(len(G1s[i+1][0][1])): #range de 3
                            
                        G1s[i+1][0][1][k] = abs(self.fontes[i+1]-self.xDataCamada1[i+1][1][k])
                    
                    for k in range(len(G1s[i+1][1][2])): #range de 3
                            
                        G1s[i+1][1][2][k] = abs(self.fontes[i+1]-self.xDataCamada1[i+1][2][k])

            print(G1s)
            
            for i in range(len(self.xDataCamada1)):
                if i+1 not in self.especiais:
                    if len(np.array(G1s[i+1])) > 0:
                        m1s.append(np.linalg.lstsq(np.array(G1s[i+1]),np.array(self.yDataCamada1[i+1]))[0])
                else:
                    m1s.append(np.linalg.lstsq(np.array(G1s[i+1][0][1]),np.array(self.yDataCamada1[i+1][1]))[0])
                    m1s.append(np.linalg.lstsq(np.array(G1s[i+1][1][2]),np.array(self.yDataCamada1[i+1][2]))[0])
            ...
            self.ax_model = self.fig_model.add_subplot(111)   
            self.ax_model.plot(np.array(temp)[0],np.array(temp)[-1], color = '#5bdaff', label = str(int(round(1/m1*1000)))+' m/s',linewidth = 3)
            l_model, = self.ax_model.plot(np.array(temp),-1*(best_fit[len(self.fontes):-1]*(1/m1)*(1/best_fit[-1]))/np.sqrt(((1/best_fit[-1])**2)-(1/m1)**2),
                          label = str(int(round(1/best_fit[-1]*1000)))+' m/s', color = '#94ff2e', linewidth = 3)
            self.ax_model.set_ylabel('Elevação (m)')
            self.ax_model.set_xlabel('Distância (m)')
            self.ax_model.set_xlim(np.array(temp)[0],np.array(temp)[-1])
            self.ax_model.set_ylim(min(l_model.get_ydata())-5,5)
            self.ax_model.fill_between(np.array(temp),-1*(best_fit[len(self.fontes):-1]*(1/m1)*(1/best_fit[-1]))/np.sqrt(((1/best_fit[-1])**2)-(1/m1)**2),
                                  0, color = '#5bdaff')
            self.ax_model.fill_between(np.array(temp),-1*(best_fit[len(self.fontes):-1]*(1/m1)*(1/best_fit[-1]))/np.sqrt(((1/best_fit[-1])**2)-(1/m1)**2),
                                  min(l_model.get_ydata())*2, color = '#94ff2e')
            self.ax_model.grid()
            ...
            self.modeloExiste = True
        ###
            if messagebox.askyesno("Refrapy - Sisref", "Visualizar a seleção de soluções?"):
                ...
                for m in best_fits:
                    ...
                    esp, = ax_solus_auto.plot(np.array(temp),
                             -1*(m[len(self.fontes):-1]*(1/m1)*(1/m[-1]))/np.sqrt(((1/m[-1])**2)-(1/m1)**2),
                                       linewidth = 0.8, linestyle = '--')
                    espessuras.append(min(esp.get_ydata()))
                ...
                print(-1*(best_fit[len(self.fontes):-1]*(1/m1)*(1/best_fit[-1]))/np.sqrt(((1/best_fit[-1])**2)-(1/m1)**2),)
                #print(m1s)
                plt.gca().set_aspect('equal', adjustable='box')
                plt.draw()
